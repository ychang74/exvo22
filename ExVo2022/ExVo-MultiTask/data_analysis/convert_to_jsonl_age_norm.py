...
max_age = float(df['age'][df['Split'] != 'Test'].max())

print(max_age)

min_age = float(df['age'][df['Split'] != 'Test'].min())

print(min_age)

df['age'][df['Split'] != 'Test'] = df['age'][df['Split'] != 'Test'].apply(lambda x: (x - min_age) / (max_age - min_age))
...
write_jsonl_into_file(df.to_dict(orient='records'), 'data_age_norm.jsonl')
write_jsonl_into_file(train_df.to_dict(orient='records'), 'train_age_norm.jsonl')
